...
namespace Revelation.Projectiles.武器弹幕.衰竭辐射
{
	public class 污浊骑士弹幕 : ModProjectile
	{
		...
		public override void SetDefaults() {
			Projectile.width = 10; // 该射弹碰撞箱的宽度
			Projectile.height = 10; // 该射弹碰撞箱的高度
			...
			Projectile.penetrate = 1; // 射弹的穿透数量 (OnTileCollide 为true时命中物块反弹也会减少一次穿透数量)
			Projectile.timeLeft = 240; // 射弹存在的剩余时间，写在这里为射弹的最大存在时间 (60 为 1 秒, 这里是 10 秒)
			Projectile.alpha = 0 ; // 射弹的透明度，255为完全透明，因为我们上面用来 aiStyle 为1，其中的一项特性就是自动淡入子弹，
									// 既子弹会由透明转为不透明（255→0），如果你没有使用 aiStyle = 1，那这里应该为 0 来使射弹完全不透明
									// 你也可以在 ai 中不断改变该值来改变射弹的透明度，但除非你确实想让射弹完全不可见，最好别让该值长时间停留在255
			Projectile.light = 4f; // 射弹发光亮度
